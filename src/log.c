 ...
 *
 *        Version:  0.0.1
 *        Created:  12/13/2012 08:48:38 PM
 *       Revision:  r1
...
int log_recovery(struct log *l, struct skiplist *list)
{
	...
	/* 
	 * Get the two log files:new and old_log
	...
	if((flag & 0x01) == 0x01) {
		...
		__DEBUG(LEVEL_DEBUG, "prepare to revovery from new log#%s", l->log_new);
	...
	if((flag & 0x10) == 0x10) {
		...
		snprintf(l->log_old, FILE_PATH_SIZE, "%s/%s", l->basedir, l->log_old);
